"""Integration tests for remote ls and exec commands in Tiny SHell."""

import subprocess


def test_ls_root_and_home(tshd_daemon):
    """Test remote directory listing on / and /home."""
    config = tshd_daemon

    # Test ls /
    res_root = subprocess.run(
        [config["tsh_path"], "-s", config["secret"], "localhost", "ls", "/"],
        capture_output=True,
        text=True,
        check=False,
    )
    assert res_root.returncode == 0
    assert "etc" in res_root.stdout

    # Test ls /home
    res_home = subprocess.run(
        [config["tsh_path"], "-s", config["secret"], "localhost", "ls", "/home"],
        capture_output=True,
        text=True,
        check=False,
    )
    assert res_home.returncode == 0


def test_exec_touch_verification(tshd_daemon):
    """Verify remote execution creates a file and remote ls reflects it."""
    config = tshd_daemon
    target_file = "/var/tmp/touched_by_pytest"

    # Ensure file doesn't exist first (via exec rm)
    subprocess.run(
        [
            config["tsh_path"],
            "-s",
            config["secret"],
            "localhost",
            "exec",
            f"/bin/rm -f {target_file}",
        ],
        check=False,
    )

    # Run touch
    res_touch = subprocess.run(
        [
            config["tsh_path"],
            "-s",
            config["secret"],
            "localhost",
            "exec",
            f"/usr/bin/touch {target_file}",
        ],
        capture_output=True,
        text=True,
        check=False,
    )
    assert "Exit code: 0" in res_touch.stdout

    # Verify via ls
    res_ls = subprocess.run(
        [config["tsh_path"], "-s", config["secret"], "localhost", "ls", "/var/tmp"],
        capture_output=True,
        text=True,
        check=False,
    )
    assert "touched_by_pytest" in res_ls.stdout

    # Cleanup
    subprocess.run(
        [
            config["tsh_path"],
            "-s",
            config["secret"],
            "localhost",
            "exec",
            f"/bin/rm -f {target_file}",
        ],
        check=False,
    )
