"""Edge case tests for filenames and paths containing unusual or gross characters."""
...
def test_gross_filename_integrity(tshd_daemon, tmp_path, gross_name):
    ...
    try:
        ...
        # 4. LS
        res = subprocess.run(
            [config["tsh_path"], "-s", config["secret"], "localhost", "ls", remote_dir],
            capture_output=True,
            text=True,
            check=False,
        )
        ...
        assert (
            gross_name in res.stdout
        ), f"Filename {repr(gross_name)} not found in ls output:\n{res.stdout}"
...
def test_gross_directory_creation(tshd_daemon):
    ...
    try:
        # Test LS
        res = subprocess.run(
            [config["tsh_path"], "-s", config["secret"], "localhost", "ls", gross_dir],
            capture_output=True,
            text=True,
            check=False,
        )
